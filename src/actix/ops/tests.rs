...
use crate::{
    CursorPage, Direction, Error, FromRelationships, FromRequest, Identifier, ListQuery, Range,
    ResourceResponse, ResourceType, Response, ShowQuery, StringMatch, Total, ID,
};
...
impl Show for WidgetStore {
    ...
    async fn show(&self, _ctx: (), id: IdOf<Self>, _q: ShowQuery) -> Result<Self::Shown, Error> {
        self.inner
            .read()
            .unwrap()
            .get(&id)
